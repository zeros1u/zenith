...
The verifier reports identification, interception result, hit time, minimum separation, and range-estimation MAE/RMSE. The automated suite additionally checks all 96 edge directions, green-only fill semantics, diagonal failure despite `4/4` cardinals, weighted-point limits, camera-crossing invalidation, detector-pose isolation, saved-frame two-second containment across every drone behavior, every drone as a baseline interceptor, half-speed no-lock cruise, downward vectored thrust, onboard-only boresight, finite rocket burnout/RCS, both rockets as interceptors and targets, visual lock loss/reacquisition, gravity/aerodynamics, and every default interception scenario.
...
There is no trained neural image-recognition model in this milestone. The generic visual detector is a deterministic synthetic-camera adapter: simulation truth is used inside that adapter to emit the 2D box, bearing, confidence, and imperfect pose estimate that real detection/pose models would provide. Defense estimation and guidance consume only that output. Model identity deliberately does **not** come from the detector; it becomes available through the simulated post-detection signal lookup described in the project idea. The boundary is ready for a later YOLO plus pose-estimation adapter without changing the range, tracking, guidance, or HUD layers.
