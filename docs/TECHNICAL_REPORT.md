...
The optional multi-contact setup creates up to three intruders. The detector, signal timer, pinhole range estimate, filtered metric track, uncertainty, prediction ovals, and guidance solution are stored separately for every contact. Priority is scored from apparent size, detector confidence, estimated range, estimated closing speed, and estimated time to contact. A score advantage and 0.35 s dwell are required while the current contact remains visible; if it is lost and another contact is visible, selection can switch immediately. Exact simulated target coordinates are not inputs to this choice.
...
The camera remains rigidly attached to the airframe. AEGIS-Q4 and SMART EVADER use a published 6° upward mount cant and 24° maximum body tilt. This is a fixed geometry/flight-envelope constraint, not a truth-following gimbal: sensor direction is calculated only from vehicle attitude plus the constant mount transform.
