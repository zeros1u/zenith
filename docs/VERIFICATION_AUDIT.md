...
| End-to-end interception | All seven default verifier scenarios identify and physically contact the assigned threat. Every drone succeeds as an interceptor in the baseline evasive case; every drone target and both incoming rockets are covered by the standard catalogue tests. | `verify_prototype.py --duration 30` and the scenario/catalogue suite. |
...
The verification inset and truth coordinates are simulation-only audit tools. Neither can feed the detector, range estimate, target track, oval construction, or defense maneuver command. The explicitly labeled `TRICKY AI` scenario separately permits the adversary's own controller to observe the interceptor because that is the test assumption.
