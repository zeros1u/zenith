...
| Reachability color | Every passing edge segment is green and every failing segment is red. Only a fully green `96/96` loop is selectable; a partial loop is mixed, and grey means no finite valid bound. | Complete-edge color, 96-segment rendering, and unbounded-prediction tests. |
...
| End-to-end interception | All seven scenarios identify and physically contact the assigned threat; every drone target and both incoming rockets are covered. | `verify_prototype.py --duration 30` and the complete scenario/catalogue suite. |
